...
namespace hand_detector
{
    ...
    private:
        ...
        // IMX500 native postprocessing (PoseNet)
        EnhancedHandDetection create_hand_from_wrist(
            const camera::IMX500Keypoint &wrist,
            const camera::IMX500PoseDetection &pose,
            uint32_t frame_width, uint32_t frame_height,
            bool is_right_hand);

        // IMX500 hand landmarks (21 keypoints, accurate finger counting)
        EnhancedHandDetection create_hand_from_landmarks(
            const camera::IMX500HandLandmark &hand_data,
            uint32_t frame_width, uint32_t frame_height);

        int count_fingers_from_landmarks(
