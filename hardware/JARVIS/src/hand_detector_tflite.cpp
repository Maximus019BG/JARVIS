...
bool TFLiteHandDetector::init(const TFLiteConfig& config) {
    ...
    if (config.verbose) {
        ...
        for (int i = 0; i < input_tensor->dims->size; i++) {
            ...
            if (i < input_tensor->dims->size - 1) std::cerr << ", ";
        }
        std::cerr << "]\n";
...
std::vector<HandDetection> TFLiteHandDetector::detect(const camera::Frame& frame) {
    ...
    if (!input_tensor) {
        if (impl_->config.verbose) {
            std::cerr << "[TFLiteHandDetector] ERROR: No input tensor\n";
        }
        return results;
    }
    ...
    // Prepare input data (resize and normalize)
    uint8_t* input_data = input_tensor->data.uint8;
    if (input_tensor->type == kTfLiteFloat32) {
        prepare_input_float(frame, reinterpret_cast<float*>(input_tensor->data.f),
    ...
    if (impl_->interpreter->Invoke() != kTfLiteOk) {
        ...
        if (impl_->config.verbose) {
            std::cerr << "[TFLiteHandDetector] ERROR: Inference failed\n";
        }
        return results;
    }
    ...
    impl_->stats.avg_inference_ms = 
        (impl_->stats.avg_inference_ms * 0.9f) + (inference_ms * 0.1f);
    
    // Parse output
    // Typical MediaPipe hand landmark model outputs:
    // - landmarks: [1, 21, 3] (x, y, z for 21 points)
    // - hand_flag: [1, 1] (detection confidence)
    // - handedness: [1, 1] (left/right classification)
    
    auto* landmarks_tensor = impl_->interpreter->output_tensor(0);
    auto* confidence_tensor = impl_->interpreter->outputs().size() > 1 ? 
                             impl_->interpreter->output_tensor(1) : nullptr;
    ...
    // Get hand presence confidence
    float hand_confidence = 1.0f;
    if (confidence_tensor && confidence_tensor->data.f) {
        hand_confidence = confidence_tensor->data.f[0];
    }
    
    if (hand_confidence < impl_->config.min_detection_confidence) {
        return results;
    }
    ...
    for (int i = 0; i < 21; i++) {
        ...
        float z = landmarks[i * 3 + 2]; // depth (not used for 2D drawing)
        
        Point landmark_point(
            static_cast<int>(x * frame.width),
            static_cast<int>(y * frame.height)
        );
        detection.landmarks.push_back(landmark_point);
    }
    ...
    detection.gesture_confidence = hand_confidence;
    detection.num_fingers = count_extended_fingers(detection);
    
    // Debug logging (every 30 frames)
    if (impl_->config.verbose && impl_->stats.total_inferences % 30 == 0) {
        std::cerr << "[HandDetect] Fingers: " << detection.num_fingers 
                  << " Gesture: " << static_cast<int>(detection.gesture)
                  << " (0=Unknown, 1=Palm, 2=Fist, 3=Pointing, 4=Thumbs, 5=Peace)"
    ...
    // Smooth index fingertip position for drawing
    ...
    impl_->confidence_buffer.push(hand_confidence);
    ...
    if (impl_->config.verbose && impl_->stats.total_inferences % 30 == 0) {
        std::cerr << "[TFLiteHandDetector] Stats: "
                  << "Inferences=" << impl_->stats.total_inferences
                  << " Success=" << impl_->stats.successful_detections
                  << " Avg_ms=" << impl_->stats.avg_inference_ms
                  << " Avg_conf=" << impl_->stats.avg_confidence
                  << "\n";
    }
...
bool TFLiteHandDetector::is_finger_extended(const HandDetection& detection, int tip_idx) const {
    if (detection.landmarks.size() < 21) return false;
...
int TFLiteHandDetector::count_extended_fingers(const HandDetection& detection) const {
    ...
    if (is_finger_extended(detection, THUMB_TIP)) count++;
    if (is_finger_extended(detection, INDEX_FINGER_TIP)) count++;
    if (is_finger_extended(detection, MIDDLE_FINGER_TIP)) count++;
    if (is_finger_extended(detection, RING_FINGER_TIP)) count++;
    if (is_finger_extended(detection, PINKY_TIP)) count++;
...
DetectionStats TFLiteHandDetector::get_stats() const {
    ...
    stats.avg_processing_time_ms = impl_->stats.avg_inference_ms;
    stats.avg_confidence = impl_->stats.avg_confidence;
#endif
    return stats;
}
