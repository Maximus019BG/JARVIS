...
/**
 * "What may this device reach" — the whole answer, in one form. Replaces a device's whole
 * grant set rather than merging, so unticking a box actually removes access — see the PATCH
 * handler.
 *
 * Knows nothing about where it is rendered. The settings page floats it in the global sheet;
 * the MCP token manager shows it in place, because that manager may itself be inside a dialog
 * and a sheet over a modal traps focus in the wrong layer. Hence `onCancel`/`onSaved` rather
 * than a `sheet.close()` baked in.
 */
export function DeviceAccessForm({
  device,
  blueprints,
  onCancel,
  onSaved,
  ...
  const save = async () => {
    setSaving(true);
    try {
      // Two independent boundaries, saved together because the form asks about both:
