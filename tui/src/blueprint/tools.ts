...
export const blueprintEditTool = (ctx: ToolContext, root: string) =>
  tool({
    description: [
      ...
      'For anything made of standard parts add an entry to `ops` whose `op` field is "place", dropping a symbol roughly where it belongs with a `label`,',
      'then another whose `op` is "connect", with from:"R1.2" and to:"U1.5", to name the two ports you want joined.',
      ...
      'An `ops` entry whose `op` is "arrange" snaps parts to the grid and separates any that overlap, so rough placement is enough.',
      'The `nets` list in `blueprint_view` format:"json" is the connectivity: which ports each wire actually joins.',
