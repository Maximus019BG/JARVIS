...
export const blueprintSymbolTool = (ctx: ToolContext, root: string) =>
  tool({
    description: [
      ...
      "Do NOT work out wire coordinates: name the two ports with `blueprint_edit` op:\"connect\", from:\"R1.2\",",
      'to:"U1.5", and the route, its junction dots and its label are worked out for you — and redone if a part moves.',
      "Placing alone already draws the connections that need no interpretation (a GND pin beside a ground symbol, a",
      "port landing on a wire, two lone parts nearest each other); the reply lists them.",
