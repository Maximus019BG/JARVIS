...
export type Item =
  ...
  | { kind: "tool"; id: string; name: string; input: unknown; output?: string; failed?: boolean; agent?: string }
 ...
 */
export function applyEvent(items: Item[], event: AgentEvent, agent?: string): Item[] {
  switch (event.type) {
    case "text": {
    ...
    case "tool-start":
      return [...items, { kind: "tool", id: event.id, name: event.name, input: event.input, agent }]
    case "tool-end": {
      const index = items.findLastIndex((item) => item.kind === "tool" && item.id === event.id)
      if (index === -1) {
        return [...items, { kind: "tool", id: event.id, name: event.name, input: {}, output: event.output }]
      }
      const updated = [...items]
      updated[index] = { ...(items[index] as Extract<Item, { kind: "tool" }>), output: event.output, failed: event.failed }
      return updated
    }
    case "error":
      return [...items, { kind: "note", text: event.message, level: "error" }]
    case "sub":
      return applyEvent(items, event.event, event.agent)
    default:
      // reasoning and usage do not produce transcript items
      return items
  }
}
